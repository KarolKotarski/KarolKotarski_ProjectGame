...
unsigned int widthh13, heightt13;
GLuint textureID13;
char *loadBMP_13()
{
	...
	// Open the file
	FILE * file = fopen("zakonczenie.bmp", "rb");
	if (!file)
	{
	...
	else
	{
		...
		else
		{
			...
			else
			{
				...
				widthh13 = *(int*)&(header[0x12]);
				heightt13 = *(int*)&(header[0x16]);

				// Some BMP file nie maja wewnych danych
				if (imageSize == 0)    imageSize = widthh13*heightt13 * 3; // 3 : one byte for each Red, Green and Blue component

				cout << "\npozycja aktualna dane: " << dataPos << endl << "wielkosc obrazka " << imageSize << endl << "szerokosc " << widthh13 << endl << "wysokosc " << heightt13 << endl << endl;
...
void CWygrana::rysujWygrana()
{
	glEnable(GL_TEXTURE_2D);
	glBindTexture(GL_TEXTURE_2D, textureID13);
	glTexImage2D(GL_TEXTURE_2D, 0, GL_RGB, widthh13, heightt13, 0, GL_RGB, GL_UNSIGNED_BYTE, dane13);
